...
duracao_total = 8.0  # Tempo em segundos para dar a volta completa
...
# Novos Pontos de Controle para um 8 Simples
# cada S possuí 4 pontos de controle para a curva cúbica, totalizando 16 pontos para os 4 segmentos do 8.
# Segmento 1: Saindo do centro (baixo) para o loop direito
S1 = [np.array([0, 0, 0]), np.array([15, 0, 15]), np.array([30, 0, 15]), np.array([30, 1, 0])]
# Segmento 2: Voltando do loop direito para o centro (alto)
S2 = [np.array([30, 1, 0]), np.array([30, 2, -15]), np.array([15, 3, -15]), np.array([0, 4, 0])]
# Segmento 3: Saindo do centro (alto) para o loop esquerdo
S3 = [np.array([0, 4, 0]), np.array([-15, 5, 15]), np.array([-30, 3, 15]), np.array([-30, 2, 0])]
# Segmento 4: Voltando do loop esquerdo para o centro (baixo)
S4 = [np.array([-30, 2, 0]), np.array([-30, 1, -15]), np.array([-15, 0, -15]), np.array([0, 0, 0])]

Todos_Segmentos = [S1, S2, S3, S4]
...
def inicializaPista():
    ...
    res = 60 # Equivalente à resolução/precisão da curva
    
    for seg in Todos_Segmentos:
            for i in range(res):
                t = i / float(res)
                pos = calculaPosicaoBezierCubica(seg[0], seg[1], seg[2], seg[3], t)
                tan = calculaTangenteBezierCubica(seg[0], seg[1], seg[2], seg[3], t)
                ang = np.degrees(np.arctan2(tan[0], tan[2]))
                # Adicionamos também o 'Pitch' se quiser que o carro incline na subida
                ang_pitch = np.degrees(np.arcsin(-tan[1])) 
                Dados_Pista.append((pos, ang, ang_pitch))
...
def inicializaRenderizacao():
    ...
    while not glfw.window_should_close(Window):
        ...
        t_global = (tempo_atual % duracao_total) / duracao_total * 4.0
        
        # Descobre em qual segmento o carro está e o t local (0.0 a 1.0)
        indice_seg = int(t_global) % 4
        t_local = t_global - int(t_global)
        
        seg_atual = Todos_Segmentos[indice_seg]
        
        pos_carro = calculaPosicaoBezierCubica(seg_atual[0], seg_atual[1], seg_atual[2], seg_atual[3], t_local)
        tan_carro = calculaTangenteBezierCubica(seg_atual[0], seg_atual[1], seg_atual[2], seg_atual[3], t_local)
        
        ry_carro = np.degrees(np.arctan2(tan_carro[0], tan_carro[2]))
        rx_carro = np.degrees(np.arcsin(-tan_carro[1])) # Inclinação para subidas
        ...
        for dados in Dados_Pista:
            posicao = dados[0]
            angulo_ry = dados[1]
            angulo_rx = dados[2] # Para inclinar o carro nas subidas/descidas
            ...
            transformacaoGenerica(posicao[0], posicao[1], posicao[2], 
                                  8.0, 0.2, 1.5, 
                                  angulo_rx, angulo_ry, 0.0)
            glDrawArrays(GL_TRIANGLES, 0, 36)

        # RENDERIZAÇÃO DOS PONTOS DE CONTROLE (Visualização Didática)
        ...
        for seg in Todos_Segmentos:
            for i, pc in enumerate(seg):
                if i == 0 or i == 3:
                    # ancoras (inicio e fim do segmento)
        ...
        defineCor(0.0, 1.0, 0.5, 1.0) 
        transformacaoGenerica(pos_carro[0], pos_carro[1] + 0.7, pos_carro[2], 
                            2.0, 1.0, 4.0, # Carro mais comprido em Z
                            rx_carro, ry_carro, 0.0)
        glDrawArrays(GL_TRIANGLES, 0, 36)
